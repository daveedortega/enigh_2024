...
cuts <- svyquantile(~ingreso_mensual_pp, survey_trabajadores,
                    quantiles = c(0.01, 0.99), na.rm = TRUE)
...
Parece poco justo mantener a aquellos que, si bien están ocupados, no percibieron un salario, por lo que revisemos la distribución para aquellos **trabajadores que percibieron un salario**.

## Distribución del Ingreso Laboral en México para Trabajadores que Percibieron un salario

Vale la pena volver a calcular los deciles, dado que estamos cambiando el universo, en principio se antoja extraño que el

```{r, eval=T, echo=TRUE}
survey_trabajadores %>% as_survey() %>% mutate(trabajo_y_no_recibio = ifelse(trabajo==0,1,0)) %>% 
  summarise(survey_mean(trabajo_y_no_recibio)) 
```

**14.5% de los hogares con personas que trabajan NO TUVIERON UN INGRESO LABORAL**, esto puede deberse a la definición de ocupado que usa el INEGI, donde las personas que buscan trabajo se encuentran ocupadas, según ellos.

```{r, eval=T, echo=TRUE}
concentrado_trabajadores_ct <- concentrado_trabajadores %>% filter(trabajo>0)
survey_trabajadores <- svydesign(ids = ~folioviv, weights = ~factor, data = concentrado_trabajadores_ct)
deciles <- svyquantile(~ingreso_mensual_pp, survey_trabajadores, quantiles = seq(0.1, 0.9, 0.1), na.rm = TRUE)
